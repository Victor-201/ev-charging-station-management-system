...
exports.confirmReservation = async (req, res) => {
  try {
    ...
    const result = await BookingService.confirmReservation(reservation_id, {
      payment_info,
    });
...
exports.markPaymentFailed = async (req, res) => {
  try {
    ...
    const result = await BookingService.markPaymentFailed(reservation_id, {
      cancel,
      reason,
    });
...
exports.updateStatus = async (req, res) => {
  try {
    ...
    if (!waitlist_id)
      return res.status(400).json({ error: "Missing waitlist_id" });
    if (!status) return res.status(400).json({ error: "Missing status" });

    const updated = await BookingService.updateWaitlistStatus(
      waitlist_id,
      status
    );
    return res.json(updated);
...
exports.checkPrice = async (req, res) => {
  try {
    const { point_id, start_time, end_time } = req.body;

    // Validate input
    if (!point_id || !start_time || !end_time) {
      return res.status(400).json({
        message: "Missing required fields: point_id, start_time, end_time",
      });
    }
...
exports.previewCost = async (req, res) => {
  try {
    ...
    const result = await BookingService.previewReserrvationCost(
      reservation_id,
      { roundUp: true }
    );
    return res.json(result);
...
exports.calculateCost = async (req, res) => {
  try {
    ...
    const result = await BookingService.calculateReservationCost(
      reservation_id,
      {
        roundUp,
...
exports.calculatePricing = async (req, res) => {
  try {
    ...
    const price = await BookingService.calculatePricing(
      point_id,
      start_time,
      end_time,
      token
...
exports.finalizeReservation = async (req, res) => {
  try {
    ...
    const result = await BookingService.finalizeReservation(reservation_id, {
      end_time,
      roundUp,
    });
...
exports.calculateReservation = async (req, res) => {
  try {
    ...
    const roundUp =
      req.body.roundUp !== undefined ? Boolean(req.body.roundUp) : true;
    const payment_method = req.body.payment_method || null;

    const result = await BookingService.calculateReservationCost(
      reservation_id,
      { roundUp, payment_method }
    );
...
exports.refun = async (req, res) => {
  try {
    ...
    if (
      autoExecutePayment &&
      typeof BookingService.publishPaymentAction === "function"
    ) {
      try {
